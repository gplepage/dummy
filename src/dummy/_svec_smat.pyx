...
cdef class svec:
    ...
    def __getstate__(self):
        ...
        # cdef numpy.ndarray[double, ndim=1] val = numpy.empty(self.size, FLOAT_TYPE)
        _idx = numpy.empty(self.size, numpy.intp)
        cdef Py_ssize_t[::1] idx = _idx
        _val = numpy.empty(self.size, float)
        cdef double[::1] val = _val
        cdef Py_ssize_t i
    ...
    cpdef object indices(self):
        cdef Py_ssize_t i
        _ans = numpy.zeros(self.size, numpy.intp)
        cdef Py_ssize_t[::1] ans = _ans
        for i in range(self.size):
            ans[i] = self.v[i].i
        return _ans
    ...
    cpdef object values(self):
        cdef Py_ssize_t i
        _ans = numpy.zeros(self.size, float)
        cdef double[::1] ans = _ans
        for i in range(self.size):
            ans[i] = self.v[i].v
        return _ans    
...
cdef class smat:
    ...
    cpdef object append_diag(self, const double[:] d):
        ...
        _vrange = numpy.arange(nr, nr+d.shape[0], dtype=numpy.intp)
        vrange = _vrange 
    ...
    cpdef object toarray(self):
        ...
        cdef Py_ssize_t i, j
        _ans = numpy.zeros((nr,nr),FLOAT_TYPE)
        ans = _ans
        for i in range(nr):
